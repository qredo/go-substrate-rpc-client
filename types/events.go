...
func (d *DemocracyVote) Decode(decoder scale.Decoder) error {
	b, err := decoder.ReadOneByte()
	d.Aye = (128 & b) == 128
	convictionBits := (127 & b)
	vb := DemocracyConviction(convictionBits)
	switch vb {
	case None, Locked1x, Locked2x, Locked3x, Locked4x, Locked5x, Locked6x:
		d.Conviction = vb
	default:
...
// EventCouncilVoted is emitted when a motion (given hash) has been voted on by given account, leaving
...
// EventTechnicalMembershipDummy is emitted when - phantom member, never used.
